...
std::vector<UnwrappedTileID> tileCover(Point<double> tl,
    ...
    // Add 0.1 to account for radians/degrees precision loss.
    constexpr double maxPitchForUnlimitedCoverage = (60.0 * DEG2RAD) + 0.1;

    // Clamp the tile coverage area to the nearest 4 neighbor tiles, in any
    // axis (we take bearing in consideration to ensure that).
    if (pitch > maxPitchForUnlimitedCoverage) {
        ...
        auto limitTileCoordinate = [&](Point<double>& point) {
            ...
            auto clampAxis = [](double a, double b) -> double {
                return util::clamp(a, a >= b ? b : b - 4.0, a >= b ? b + 4.0 : b);
            };
