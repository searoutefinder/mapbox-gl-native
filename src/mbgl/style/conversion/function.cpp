...
FunctionType functionType(type::Type type, const Convertible& value) {
    ...
    if (*string == "exponential" && interpolatable)
...
optional<double> convertBase(const Convertible& value, Error& error) {
...
std::unique_ptr<Expression> step(type::Type type, std::unique_ptr<Expression> input, std::map<double, std::unique_ptr<Expression>> stops) {
    return std::make_unique<Step>(type, std::move(input), std::move(stops));
}

std::unique_ptr<Expression> interpolate(type::Type type, Interpolator interpolator, std::unique_ptr<Expression> input, std::map<double, std::unique_ptr<Expression>> stops) {
    ParsingContext ctx;
...
template <class T>
std::unique_ptr<Expression> match(type::Type type, const std::string& property, std::map<T, std::unique_ptr<Expression>> branches) {
...
template <>
std::unique_ptr<Expression> match<bool>(type::Type type, const std::string& property, std::map<bool, std::unique_ptr<Expression>> branches) {
...
optional<std::unique_ptr<Expression>> convertCategoricalFunction(type::Type type,
    ...
    if (toBool(arrayMember(first, 0))) {
        ...
        return match(type, property, std::move(*branches));
    }
...
optional<std::unique_ptr<Expression>> convertSourceFunctionToExpression(type::Type type,
    ...
    case FunctionType::Categorical:
        return convertCategoricalFunction(type, value, error, *propertyString);
...
optional<std::unique_ptr<Expression>> composite(type::Type type,
    ...
    for (auto& e : map) {
        stops[e.first] = makeInnerExpression(type, *base, *propertyString, std::move(e.second));
    }
    ...
    if (toBool(*sourceValue)) {
        ...
        case FunctionType::Categorical:
            return composite<bool>(type, value, err, [] (type::Type type_, double, const std::string& property, std::map<bool, std::unique_ptr<Expression>> stops) {
                return match<bool>(type_, property, std::move(stops));
            });
        default:
    ...
    if (toNumber(*sourceValue)) {
        ...
        case FunctionType::Categorical:
            return composite<int64_t>(type, value, err, [] (type::Type type_, double, const std::string& property, std::map<int64_t, std::unique_ptr<Expression>> stops) {
                return match<int64_t>(type_, property, std::move(stops));
            });
        default:
            err = { "unsupported function type" };
            return {};
    ...
    if (toString(*sourceValue)) {
        ...
        case FunctionType::Categorical:
            return composite<std::string>(type, value, err, [] (type::Type type_, double, const std::string& property, std::map<std::string, std::unique_ptr<Expression>> stops) {
                return match<std::string>(type_, property, std::move(stops));
            });
        default:
            err = { "unsupported function type" };
            return {};
